...
try:
  import jr.np_box_list as np_box_list
  import jr.np_box_list_ops as np_box_list_ops
except:
...
class PerImageEvaluation(object):
  ...
  def compute_object_detection_metrics(
      self, task, detected_boxes, detected_scores, detected_class_labels,
    ...
    # if task == 'task_2':
    #     gt_refine, det_refine, FPs = self._refine_group_ids(detected_boxes, detected_scores,
    #                                                    groundtruth_boxes, groundtruth_is_group_of_list)
    #     gt_g_id, det_g_id = [], []
    #     for idx in range(len(gt_refine)):
    #         gt_g_id.append(groundtruth_class_labels[gt_refine[idx]])
    #         det_g_id.append(detected_class_labels[det_refine[idx]])
    #     refined_det_g_id = self.refine_y_pred(np.array(gt_g_id), np.array(det_g_id))
    #
    #     for idx, d in enumerate(range(detected_class_labels.shape[0])):
    #       if d in det_refine and refined_det_g_id[idx]==gt_g_id[idx]:
    #         detected_class_labels[idx][0] = 1
    #       elif d in det_refine and refined_det_g_id[idx]!=gt_g_id[idx]:
    #         detected_class_labels[idx][0] = 0
    #       elif d in FPs:
    #         detected_class_labels[idx][0] = 1
    #
    #     for g in range(groundtruth_class_labels.shape[0]):
    #         groundtruth_class_labels[g][0] = 1
    #
    # if task == 'task_3':
    #     gt_classes_dict = {'1':[], '2':[], '3':[], '4':[], '5':[]}
    #     gt_classes = groundtruth_class_labels.reshape(-1).tolist()
    ...
    #     for idx, d in enumerate(range(detected_class_labels.shape[0])):
    #         if d in det_refine and refined_det_g_id[idx] == gt_g_id[idx]:
    #           detected_class_labels[idx][0] = [key for (key, value) in gt_classes_dict.items() if gt_g_id[idx] in value][0]
    #         elif d in det_refine and refined_det_g_id[idx] != gt_g_id[idx]:
    #             detected_class_labels[idx][0] = 0
    #         elif d in FPs:
    #             detected_class_labels[idx][0] = \
    #             [key for (key, value) in gt_classes_dict.items() if gt_g_id[idx] in value][0]
    #
    #
    #     for g in range(groundtruth_class_labels.shape[0]):
    #         groundtruth_class_labels[g][0] = [key for (key, value) in gt_classes_dict.items() if gt_g_id[idx] in value][0]
    #
    # if task =='task_5':
    #     gt_refine, det_refine, _ = self._refine_group_ids(detected_boxes, detected_scores,
    #                                                    groundtruth_boxes, groundtruth_is_group_of_list)
    #     gt_g_id, det_g_id = [], []
    #     for idx in range(len(gt_refine)):
    #         gt_g_id.append(groundtruth_class_labels[gt_refine[idx]])
    ...
    #         if d in det_refine and refined_det_g_id[idx] == gt_g_id[idx]:
    #             detected_class_labels[idx][0] = \
    #             str([key for (key, value) in gt_classes_dict.items() if gt_g_id[idx] in value][0])+str(detected_class_labels[idx][0])
    #         elif d in det_refine and refined_det_g_id[idx] != gt_g_id[idx]:
    #             detected_class_labels[idx][0] = 0
    #         elif d in FPs:
    #             detected_class_labels[idx][0] = \
    ...
    #
    #     for g in range(groundtruth_class_labels.shape[0]):
    #         groundtruth_class_labels[g][0] = [key for (key, value) in gt_classes_dict.items() if gt_g_id[idx] in value][0]+str(groundtruth_class_labels[idx][0])
  ...
  def _compute_tp_fp_for_single_class(self, detected_boxes, detected_scores, groundtruth_boxes,
      groundtruth_is_difficult_list, groundtruth_is_group_of_list, detected_masks=None, groundtruth_masks=None):
