...
class _BaseDataset(ABC):
    ...
    @_timing.time
    def get_raw_seq_data(self, tracker, seq, is_3d):
        ...
        # print(raw_gt_data['gt_ids'][0])
        # Calculate similarities for each timestep.
        similarity_scores = []
        if is_3d:
            for t, (gt_dets_t, tracker_dets_t) in enumerate(
                zip(raw_data["gt_dets_3d"], raw_data["tracker_dets_3d"])
            ):
                de = self._calculate_similarities(gt_dets_t, tracker_dets_t)
                similarity_scores.append(de)
        else:
            for t, (gt_dets_t, tracker_dets_t) in enumerate(
    ...
    @staticmethod
    def _calculate_box_ious_3d(
        bboxes1, bboxes2, box_format="xyzwhd", do_ioa=False, max_iou=1.0
    ):
        """ Calculates the IOU (intersection over union) between two arrays of boxes.
        Allows variable box formats ('xywh' and 'x0y0x1y1').
        If do_ioa (intersection over area) , then calculates the intersection over the area of boxes1 - this is commonly
        used to determine if detections are within crowd ignore region.
        """
        objs = np.atleast_2d(bboxes1).astype(float)
        hyps = np.atleast_2d(bboxes2).astype(float)
        ...
        for o in range(objs.shape[0]):
            for h in range(hyps.shape[0]):
                base_area = _BaseDataset.find_area(
                    _BaseDataset.clip_polygon(objs[o], hyps[h])
                )
                height = max(objs[o][1], hyps[h][1]) - min(
                    objs[o][1] - objs[o][4], hyps[h][1] - hyps[h][4]
                )
                intersect = base_area * height
                area1 = objs[o][3] * objs[o][4] * objs[o][5]
                area2 = hyps[h][3] * hyps[h][4] * hyps[h][5]
                union = area1 + area2 - intersect
                if union != 0:
                    C[o, h] = intersect / union
                else:
                    C[o, h] = 0
        return C
    
    @staticmethod
    def _calculate_traj_distance_3d(
        trajs1, trajs2,
    ):
        """ Calculates the ADE (average displacement error) between two arrays of trajectories.
        """
    ...
    def clip_polygon(box1, box2):
        ...
        def sort_points(pts, center):
            x, z = center
            sorted_pts = sorted(
                [(i, (v[0] - x, v[1] - z)) for i, v in enumerate(pts)],
                key=lambda p: get_angle((p[1][0], p[1][1])),
            )
        ...
        box2_edges = np.asarray(
            [
                (
                    -np.cos(theta),
                    -np.sin(theta),
                    l / 2 - x * np.cos(theta) - z * np.sin(theta),
                ),
                (
                    -np.sin(theta),
                    np.cos(theta),
        ...
        for i in range(len(out_vertices)):
            ...
            for j in range(i + 1, len(out_vertices)):
                if (
                    abs(out_vertices[i][0] - out_vertices[j][0]) < 1e-6
                    and abs(out_vertices[i][1] - out_vertices[j][1]) < 1e-6
                ):
                    to_remove.append(j)
        out_vertices = sorted(
            [
            ...
            key=lambda p: get_angle((p[0], p[1])),
        )
        return out_vertices
    ...
    def find_area(vertices):
        ...
        for i in range(len(vertices)):
            area += vertices[i][0] * (
                vertices[(i + 1) % len(vertices)][1] - vertices[i - 1][1]
            )
        return 0.5 * abs(area)
