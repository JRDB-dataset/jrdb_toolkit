...
class Object3dDet(object):
    ...
    def __init__(self, label_file_line):
        ...
        #self.id = data[1]
        if len(data)==16:
            data=data[:3]+[-1]+data[3:]
        self.type = data[0]
        self.truncation = data[1] 
        self.occlusion = int(
            data[2]
        )
        self.num_points = data[3]
        self.alpha = data[4]
        self.xmin = data[5]  # left
        self.ymin = data[6]  # top
        self.xmax = data[7]  # right
        self.ymax = data[8]  # bottom
        self.box2d = np.array([self.xmin, self.ymin, self.xmax, self.ymax])
        self.h = data[9]  # box height
        self.w = data[10]  # box width
        self.l = data[11]  # box length (in meters)
        #self.t = (data[12], data[13], data[14])
        # -x +h/2=y
        self.t=(data[14], -data[12],-data[13]+data[9]/2)
        self.ry = data[15]
...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        self.xmax = data[6]+data[8]  # right
        self.ymax = data[7]+data[9]  # bottom
        self.box2d = np.array([self.xmin, self.ymin, self.xmax, self.ymax])
        ...
        self.t=(data[12], -data[10],-data[11]+data[14]/2)
        self.h = data[14]  # box height
        self.w = data[13]  # box width
        self.l = data[15]  # box length (in meters)
        
        # -box['cy'],
        #                       -box['cz'] + box['h']/2,
        #                       box['cx'],
        #print('t',self.t,'hwl',self.h,self.w,self.l)
        self.ry = data[16]
        # # extract label, truncation, occlusion
...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, from_video=False):
        ...
        #print('calibs:',calibs,calib_filepath)
        self.P = calibs["P2"]
...
def read_label(label_filename,idx):
    lines = [line.rstrip() for line in open(label_filename) if int(line.split(' ')[0])==idx]
    #print('lines',lines)
    #print()
    #print('lines',lines,idx,label_filename)
    objects = [Object3d(line) for line in lines]
    return objects

def read_label_det(label_filename):
    lines = [line.rstrip() for line in open(label_filename)]
    #print('lines',lines)
    #print()
    #print('lines',lines,idx,label_filename)
    objects = [Object3dDet(line) for line in lines]
    return objects
def read_label_group(label_filename,seq_idx,idx):
    lines = [line.rstrip() for line in open(label_filename) if int(line.split(' ')[0])==seq_idx and int(line.split(' ')[1])==idx]
    objects = [Object3dGroup(line) for line in lines]
    return objects
def read_label_action(label_filename,seq_idx,idx):
    lines = [line.rstrip() for line in open(label_filename) if int(line.split(' ')[0])==seq_idx and int(line.split(' ')[1])==idx]
    objects = [Object3dAction(line) for line in lines]
    return objects
def read_label_activity(label_filename,seq_idx,idx):
    lines = [line.rstrip() for line in open(label_filename) if int(line.split(' ')[0])==seq_idx and int(line.split(' ')[1])==idx]
    objects = [Object3dActivity(line) for line in lines]
    return objects
...
def load_velo_scan(velo_filename, dtype=np.float32, n_vec=4,upper=False):
    #scan = np.fromfile(velo_filename, dtype=dtype)
    scan=pypcd.PointCloud.from_path(velo_filename).pc_data
    scan=np.stack((scan['x'],scan['y'],scan['z']),axis=1)
    global_config = yaml.safe_load(open(os.path.join('/pvol2/jrdb_dev/jrdb_website_dev/static/downloads/jrdb_test/test_dataset_without_labels/calibration', 'defaults.yaml')))
    camera_config = yaml.safe_load(open(os.path.join('/pvol2/jrdb_dev/jrdb_website_dev/static/downloads/jrdb_test/test_dataset_without_labels/calibration', 'cameras.yaml')))
    if upper:
        translation = np.asarray(global_config['calibrated']['lidar_upper_to_rgb']['translation'])
        theta = global_config['calibrated']['lidar_upper_to_rgb']['rotation'][-1]
    else:
        theta = global_config['calibrated']['lidar_lower_to_rgb']['rotation'][-1]
        translation = np.asarray(global_config['calibrated']['lidar_lower_to_rgb']['translation'])
    #scan = scan.reshape((-1, n_vec))
    rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
    scan[:, :2] = np.dot(rotation_matrix,scan[:, :2][:,:,np.newaxis])[:,:].T
    scan=scan-translation
    return scan
...
def compute_boxes_3d(objs, P):
    res=[]
    for obj in objs:
        _,box_3d=compute_box_3d(obj, P)
        res.append(box_3d)
    return np.array(res)
def compute_box_3d(obj, P):
    ...
    ry=(-obj.ry if -obj.ry < np.pi else 2 * np.pi -obj.ry)
    R = rotz(obj.ry)
    # print('reeeeeeeeeeeeeeeeeeeeeeeeeeee',R,ry)
    ...
    # print('reeeeeeeeeeeeeeeeeeeeeeeeeeee',np.vstack([x_corners, y_corners, z_corners]))
    corners_3d = np.dot(R, np.vstack([x_corners, y_corners, z_corners]))
    # print('asddddddddddddd',corners_3d)
    corners_3d[0, :] = corners_3d[0, :] + obj.t[0]
    corners_3d[1, :] = corners_3d[1, :] + obj.t[1]
    corners_3d[2, :] = corners_3d[2, :] + obj.t[2]
    # print('asddddddddddddd',corners_3d,obj.t[0],obj.t[1],obj.t[2])
    # print 'cornsers_3d: ', corners_3d
    # only draw 3d bounding box for objs in front of the camera
    
    # print(P.shape,corners_3d)
    # print(asd)
