...
def write_summary_results(summaries, cls, output_folder):
    ...
    #print([list(s['COMBINED_SEQ'].keys()) for s in summaries])
    sequences = summaries[0].keys()
    fields = ['seq'] + sum([list(s['COMBINED_SEQ'].keys()) for s in summaries], [])
    #print('header',fields)
    ...
    with open(out_file, 'w', newline='') as f:
        ...
        for seq in sorted(sequences):
            if seq == 'COMBINED_SEQ':
                continue
            writer.writerow([seq] + sum([list(s[seq].values()) for s in summaries], []))
        writer.writerow(['COMBINED'] + sum([list(s['COMBINED_SEQ'].values()) for s in summaries], []))

def write_detailed_results(details,summaries, cls, output_folder):
    """Write detailed results to file"""
