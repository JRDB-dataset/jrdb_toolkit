...
class Evaluator:
    ...
    @_timing.time
    def evaluate(self, dataset_list, metrics_list, is_3d=False):
                ...
                try:
                    ...
                    if config['USE_PARALLEL']:
                        with Pool(config['NUM_PARALLEL_CORES']) as pool:
                            _eval_sequence = partial(eval_sequence, dataset=dataset, tracker=tracker,
                                                     ...
                                                     metric_names=metric_names,is_3d=is_3d)
                    ...
                    for c_cls in res['COMBINED_SEQ'].keys():  # class_list + combined classes if calculated
                        ...
                        if config['OUTPUT_EMPTY_CLASSES'] or num_dets > 0:
                            ...
                            if config['OUTPUT_DETAILED']:
                                #print(summaries,details)
                                utils.write_detailed_results(details,summaries, c_cls, output_fol)
...
@_timing.time
def eval_sequence(seq, dataset, tracker, class_list, metrics_list, metric_names,is_3d):
    """Function for evaluating a single sequence"""

    raw_data = dataset.get_raw_seq_data(tracker, seq,is_3d)
    seq_res = {}
